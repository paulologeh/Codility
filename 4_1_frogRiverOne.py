...
def solution(X, A):
    ...
    for idx, number in enumerate(A):
        if not str(number) in leaves:
            leaves[str(number)] = idx   # store index of first instance

    earliest = -1
    N = len(leaves)
    # if the number of keys is less than  X then some leaves are missing
    if N < X:
        return earliest

    # if the key is less than X and the index is greater, update the earliest
    earliest = leaves[str(X)]
    for key in leaves:
        if int(key) < X and leaves[key] > earliest:
            earliest = leaves[key]
